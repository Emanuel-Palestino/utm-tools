...
export const ScheduleOfActivities: FC<ScheduleOfActivitiesProps> = ({
  ...
  return (
    <section className="w-full h-full text-[14px] flex flex-col justify-between">
      <div>
        <Header
          title="CRONOGRAMA DE ACTIVIDADES DE SERVICIO SOCIAL"
          landscape
        />

        <p className="text-right text-[16px] px-10 mt-6">
          FECHA: {formatedDate(date)}
        </p>

        <div className="flex flex-col gap-1 px-8 mt-4">
          <div className="flex gap-4">
            <p className="w-[230px] font-semibold text-right">
          ...
          <div className="flex gap-4 items-center">
            <p className="min-w-[230px] font-semibold text-right leading-4">
          ...
          <div className="flex gap-4">
            ...
            <p>
              {formatedDate(period.startDate)}{" "}
              <span className="inline-block w-20 text-center font-semibold">
                AL
              </span>{" "}
          ...
          <div className="flex gap-4">
            <p className="w-[230px] font-semibold flex-shrink-0 text-right">
              OBJETIVO:
            </p>
          ...
          <table className="w-full table-auto border-collapse">
            <thead className="border-t border-l border-black">
              <tr>
                <th
                  rowSpan={2}
                  className="border-b border-r border-black px-1 text-[11px]"
                >
                  NÚM.
                </th>
                <th
                  rowSpan={2}
                  className="border-b border-r border-black min-w-96"
                >
                  ACTIVIDAD
                </th>
                ...
                <th
                  colSpan={3}
                  className="text-center border-b border-r border-black"
                >
                  DEL
                </th>
                <th
                ...
                <th
                  rowSpan={2}
                  className="border-b border-r border-black px-1 text-[10px]"
                >
            ...
            <tbody className="border-l border-black">
              {activities.map((activity, index) => (
                <tr key={`activity-${index}`} className="text-center">
                  ...
                  <td className="border-b border-r border-black text-left pl-1">
                    {activity.description}
                  </td>
                  <td className="border-b border-r border-black">
                    {String(getDate(activity.startDate)).padStart(2, "0")}
                  </td>
                  <td className="border-b border-r border-black">
                    {String(getMonth(activity.startDate) + 1).padStart(2, "0")}
                  </td>
                  <td className="border-b border-r border-black">
                    {getYear(activity.startDate)}
                  </td>
                  <td className="border-b border-r border-black">
                    {String(getDate(activity.endDate)).padStart(2, "0")}
                  </td>
                  <td className="border-b border-r border-black">
                    {String(getMonth(activity.endDate) + 1).padStart(2, "0")}
                  </td>
                  <td className="border-b border-r border-black">
                    {getYear(activity.endDate)}
                  </td>
                  <td className="border-b border-r border-black">
                    {activity.hours}
                  </td>
            ...
            <tfoot>
              <tr className="text-[14px]">
                <td></td>
                <td></td>
                <td colSpan={6} className="text-right font-semibold p-1">
                  TOTAL DE HORAS:
                </td>
      ...
      <div className="flex gap-4 px-8">
        <p className="border-t-[1.5px] border-black w-60 text-center">
          Firma del Alumno
        </p>
        <p className="border-t-[1.5px] border-black w-60 text-center">
          Responsable Inmediato (firma y sello)
        </p>
      </div>
    </section>
  )
