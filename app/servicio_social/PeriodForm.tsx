...
const PeriodForm: FC<PeriodFormProps> = ({ nextForm }) => {
  const { periodData, setPeriodData } = useSocialServiceStore()
  ...
  const onDatesChange = (startTs: number, endTs: number) => {
    ...
    const totalHoursPerDay = schedules.reduce(
      (acc, [start, end]) => acc + (end - start),
      0,
    )
    setValue(
      "totalHours",
      totalHoursPerDay * differenceInBusinessDays(endTs, startTs),
    )
  }
          ...
          <Controller
            ...
            render={({ field }) => (
              <Input
                ...
                isRequired
                min={formatISO(addMonths(startDate, 6), {
                  representation: "date",
                })}
                {...field}
                value={
              ...
              <Controller
                ...
                rules={{
                  onChange: ({ target }) => {
                    ...
                    if (fields.length > 1) {
                      if (index === 0) {
                        const secondSchedule = getValues(
                          `schedules.${index + 1}`,
                        )
                ...
                render={({ field }) => (
                  <Slider
                    ...
                    getValue={(value) =>
                      Array.isArray(value)
                        ? `${value[0]}:00 hrs. a ${value[1]}:00 hrs.`
                        : ""
                    }
                    showSteps
                    {...field}
