...
const ActivitiesForm = () => {
  const {
    ...
    periodData: { schedules, startDate, endDate } = DEFAULT_PERIOD_DATA,
  ...
  const totalHoursPerDay = periodData.schedules.reduce(
      ...
      activities: activitiesData || [
        {
          ...
          hours:
            totalHoursPerDay *
            differenceInBusinessDays(periodData.endDate, periodData.startDate),
        },
      ],
    },
  ...
  const onEndDateChange = (currentEndDate: Date, index: number) => {
    ...
    setValue(
      ...
      totalHoursPerDay *
        differenceInBusinessDays(currentEndDate, currentStartDate),
    )
    ...
    const nextHours =
      totalHoursPerDay * differenceInBusinessDays(nextEndDate, nextStartDate)
    ...
    <Card>
      <CardHeader>
        <ul className="pl-6 text-sm text-gray-600 list-disc list-outside">
          ...
          <li>
            Este <b>listado de actividades</b> aparecerá únicamente en el
            documento <b>cronograma de actividades</b>. Debido a esto la{" "}
            <b>descripción</b> debe ser <b>corta.</b>
          </li>
          <li>
            <b>No es necesario</b> que el número de actividades sea igual al
            número de meses de duración del servicio social.
          </li>
        </ul>
      </CardHeader>
      ...
      {isPeriodDataComplete ? (
            ...
            <Button
              ...
              onClick={() =>
                append({
                  description: "",
                  startDate: getValues(
                    `activities.${fields.length - 1}.endDate`,
                  ),
                  endDate: periodData.endDate,
                  hours:
                    totalHoursPerDay *
                    differenceInBusinessDays(
                      periodData.endDate,
                      getValues(`activities.${fields.length - 1}.endDate`),
                    ),
                })
              }
            >
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Input
                        ...
                        onChange={(e) =>
                          field.onChange(parseISO(e.target.value).getTime())
                        }
                        value={
                          typeof field.value === "number"
                            ? formatISO(field.value, { representation: "date" })
        ...
        <CardBody>
          <p className="text-center text-2xl text-gray-700">
            Por favor completa la información del <b>periodo</b> para completar
            este formulario.
          </p>
        </CardBody>
      )}
