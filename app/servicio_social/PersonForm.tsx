...
const PersonForm: FC<PersonFormProps> = ({ nextForm }) => {
  const { personalData, setPersonalData } = useSocialServiceStore()

  const { handleSubmit, watch, register } = useForm<Person>({
    defaultValues: {
      name: "",
      paternalSurname: "",
      maternalSurname: "",
    ...
    <Card>
      <CardBody className="p-4">
        ...
        >
          ...
          <Switch {...register("isSpeakerOfIndigenousLanguage")}>
          ...
          {watch("isSpeakerOfIndigenousLanguage") && (
            <Input
              ...
              {...register("indigenousLanguage", {
                required: watch("isSpeakerOfIndigenousLanguage"),
              })}
              isRequired={watch("isSpeakerOfIndigenousLanguage")}
            />
          )}

          <Switch {...register("hasDisability")}>
