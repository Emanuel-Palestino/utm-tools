...
const FinalReportModal: FC<FinalReportModalProps> = ({
  isOpen,
  onOpenChange,
}) => {
  const {
  ...
  const generateFinalReport = async () => {
    ...
    if (
      !reports["weekly-report-1"] ||
      !reports["weekly-report-2"] ||
      !reports["weekly-report-3"]
    ) {
  ...
  const { target: finalReportTarget, createPDF: createFinalReport } =
    usePDF("Reporte Final")
  ...
  return (
    <>
          ...
          {(onClose) => (
            <>
              ...
              <ModalFooter>
                ...
                <Button color="primary" form="final_report_form" type="submit">
                  Descargar
                </Button>
              </ModalFooter>
            </>
      ...
      <PDFWrapper target={finalReportTarget}>
        <FinalReport
          data={internshipData}
          informContent={watch("description")}
        />
      </PDFWrapper>
    </>
  )
